...
| API-10 | API | BR-04, FR-18 / AC-11, AC-16 | Missing/malformed/inactive Requester context on every scoped endpoint family and cross-owner response equivalence | `400` invalid context; no existence, metadata, path, or bytes leak | `server/tests/lab-02/attachments.api.test.ts` | Pass #16 for attachment endpoints; shared-context matrix deferred to #17 |
